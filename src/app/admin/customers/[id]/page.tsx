    ...
    <main className="min-h-screen bg-[#120f0c] text-white">
      <section className="container py-8">
        ...
        {customer ? (
            ...
            <div className="grid gap-5 lg:grid-cols-[.65fr_.35fr]">
              <Card variant="dark">
                ...
                <div className="mt-5 grid gap-3 text-[var(--text)]">
                  {customer.jobs.length ? (
                    ...
                    <p className="rounded-xl border border-white/10 bg-white/5 p-4 text-white/70">No requests yet.</p>
                  )}
                </div>
              </Card>
              ...
              <div className="grid gap-5">
                  ...
                  <div className="mt-4 grid gap-3">
                    {customer.memberships.length ? (
                      customer.memberships.map((membership) => (
                        <div key={String(membership.id)} className="rounded-xl border border-white/10 bg-white/5 p-4">
                          <p className="font-black">{String(membership.plan ?? membership.plan_code ?? "Membership")}</p>
                          <p className="mt-1 text-sm text-white/60">{String(membership.status ?? "pending")}</p>
                        </div>
                      ))
                    ) : (
                      <p className="text-sm text-white/65">No membership rows yet.</p>
                    )}
                  </div>
                </Card>
                ...
                <Card variant="dark">
                  ...
                  <p className="mt-4 text-sm leading-6 text-white/70">{customer.properties.length} saved properties</p>
                  <p className="text-sm leading-6 text-white/70">{customer.vehicles.length} saved vehicles</p>
                  <div className="mt-4 grid gap-2">
                    {customer.properties.slice(0, 3).map((property) => (
                      <div key={property.id} className="rounded-xl bg-white/5 p-3 text-sm text-white/70">
                        {property.label ?? "Property"} · {property.address}
                      </div>
                    ))}
                    {customer.vehicles.slice(0, 3).map((vehicle) => (
                      <div key={vehicle.id} className="rounded-xl bg-white/5 p-3 text-sm text-white/70">
                ...
                <Card variant="dark">
                  ...
                  <p className="mt-4 text-sm leading-6 text-white/70">{customer.reviews.length} customer reviews recorded.</p>
                </Card>
              </div>
            </div>
          </div>
          ...
          <Card variant="dark">
            <p className="text-white/70">Customer not found.</p>
          </Card>
        )}
      </section>
    </main>
